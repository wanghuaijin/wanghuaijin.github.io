...
\end{aligned}
\]
We note that $|\xi_1| |\xi_2| = 1$, then the only possibility to satisfy the root condition is $|\xi_1| = |\xi_2|=1$, which leads to $z$ is the pure imaginary number. By setting $z = b\mathrm{i}$, we obtain $b\in (-1,1)$. Hence the absolute stability region is $\{z = b\mathrm{i}: b\in (-1,1)\}$.
\end{proof}
